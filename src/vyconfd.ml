...
let write_state world =
    let vc = world.Session.vyconf_config in
    let oc = open_out_gen [Open_creat; Open_wronly] 0o644 vc.state_file
    in close_out oc;
    (* cache running config; this will be unnecessary once vyconfd is
       started at boot *)
...
let () = 
  ...
  let () =
      match !reload_active_config with
      | true -> ()
      | false -> write_state world (* write systemd state file *)
  in
