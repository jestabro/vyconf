...
let rec default_dispatch_initialization 
  ?pid:((pid:int32) = 0l)
  ?config_active:((config_active:string) = "")
  ?config_proposed:((config_proposed:string) = "")
...
type dispatch_initialization_mutable = {
  mutable pid : int32;
  mutable config_active : string;
  mutable config_proposed : string;
}
...
let rec pp_dispatch_initialization fmt (v:dispatch_initialization) = 
  let pp_i fmt () =
    Pbrt.Pp.pp_record_field ~first:true "pid" Pbrt.Pp.pp_int32 fmt v.pid;
    Pbrt.Pp.pp_record_field ~first:false "config_active" Pbrt.Pp.pp_string fmt v.config_active;
    Pbrt.Pp.pp_record_field ~first:false "config_proposed" Pbrt.Pp.pp_string fmt v.config_proposed;
  in
...
let rec pp_dispatch_envelope fmt (v:dispatch_envelope) = 
  let pp_i fmt () =
    Pbrt.Pp.pp_record_field ~first:true "dispatch" pp_dispatch fmt v.dispatch;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()
...
let rec encode_pb_dispatch_initialization (v:dispatch_initialization) encoder = 
  Pbrt.Encoder.int32_as_varint v.pid encoder;
  Pbrt.Encoder.key 1 Pbrt.Varint encoder; 
  Pbrt.Encoder.string v.config_active encoder;
  Pbrt.Encoder.key 2 Pbrt.Bytes encoder; 
  Pbrt.Encoder.string v.config_proposed encoder;
...
let rec decode_pb_dispatch_initialization d =
  ...
  let config_proposed_is_set = ref false in
  let config_active_is_set = ref false in
  let pid_is_set = ref false in
  while !continue__ do
    match Pbrt.Decoder.key d with
    | None -> (
    ); continue__ := false
    | Some (1, Pbrt.Varint) -> begin
      v.pid <- Pbrt.Decoder.int32_as_varint d; pid_is_set := true;
    end
    | Some (1, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(dispatch_initialization), field(1)" pk
    | Some (2, Pbrt.Bytes) -> begin
      v.config_active <- Pbrt.Decoder.string d; config_active_is_set := true;
    end
    | Some (2, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(dispatch_initialization), field(2)" pk
    | Some (3, Pbrt.Bytes) -> begin
      v.config_proposed <- Pbrt.Decoder.string d; config_proposed_is_set := true;
    end
    | Some (3, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(dispatch_initialization), field(3)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  begin if not !config_proposed_is_set then Pbrt.Decoder.missing_field "config_proposed" end;
  begin if not !config_active_is_set then Pbrt.Decoder.missing_field "config_active" end;
  begin if not !pid_is_set then Pbrt.Decoder.missing_field "pid" end;
  ({
    pid = v.pid;
