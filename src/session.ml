...
let get_proposed_config w s =
    ...
    apply_changes w s.changeset.delete c |>
    (apply_changes w s.changeset.add)
...
let update_set w changeset path =
    ...
    { changeset with add = (op :: changeset.add) }

let update_delete w changeset path =
    let path, value = split_path w path in
    let op = CfgDelete (path, value) in
    { changeset with delete = (op :: changeset.delete) }
...
let get_changeset w lt rt =
    ...
    let changeset = { add = []; delete = []; } in
    let add_changeset =
        List.fold_left (update_set w) changeset (CT.value_paths_of_tree add_tree)
    in
    let del_changeset =
        List.fold_left (update_delete w) changeset (CT.value_paths_of_tree del_tree)
    in
    { add = add_changeset.add; delete = del_changeset.delete; }
