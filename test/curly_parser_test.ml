...
let parse s = Vyos1x.Parser.from_string s

let raises_parsing_exception f =
    try ignore @@ f (); false
    with Vyos1x.Util.Syntax_error _ -> true
...
let test_parse_empty test_ctxt =
    ...
    assert_equal (VT.list_children config) []
...
let test_parse_with_leaf_valueless test_ctxt =
    ...
    assert_equal (VT.get config ["foo"] |> VT.list_children) ["bar"]

(* N.B. the following two cases _are_ allowed: cf. nosetests vyos-1x *)

(* Top level leaf nodes are not allowed *)
let test_parse_top_level_leaf_node test_ctxt =
    assert_bool "Top level leaf nodes was not caught by parser"
      (raises_parsing_exception (fun () -> parse config_leaf_top_level))

(* Top level tag nodes are not allowed *)
let test_parse_top_level_tag_node test_ctxt =
    assert_bool "Top level tag node was not caught by parser"
      (raises_parsing_exception (fun () -> parse config_tag_top_level))
...
let test_parse_with_tag test_ctxt =
    ...
    assert_equal (VT.get config ["foo"; "bar"] |> VT.list_children) ["baz"; "qwerty"];
...
let test_parse_with_tag_nonalpha test_ctxt =
    ...
    assert_equal (VT.get config ["foo"; "bar"] |> VT.list_children) ["baz0.99"; "baz1-8"]
